...
import ru.capjack.tool.csi.core.server.internal.*
...
class Server(
	...
	private inner class Connections : ConnectionAcceptor, ConnectionReleaser,
		ReceptionConnectionProcessorHeir {
		...
		override fun acceptConnection(connection: Connection): ConnectionHandler {
			...
			synchronized(runningLock) {
				if (running) {
					...
					val delegate =
						ConnectionDelegateImpl(connection, this, receptionConnectionProcessor, executor, connectionActivityTimeoutMillis)
